...
| Chat POST | Write-gate bypass | Allowlist: `GET /`, `/api/status`, `/api/sessions`; `POST /api/sessions`, `/api/chat`, `/api/auth/login`. No `/api/agent/*`, no `"loop"` field, no `/api/auth/logout`, `/password`, or `/users`. CI greps the client source. | New methods must extend the allowlist and tests. |
...
| Harness login | Credential handling | `Harness Login…` collects username/password via a native `NSSecureTextField` modal on the main thread; both are handed once to `Client::login` (`POST /api/auth/login`, no CSRF on that route by harness design) and dropped — never persisted to disk, never logged (`ClientError` messages carry the harness's own error code, not the credentials). The resulting session cookie lives only in that run's in-memory `reqwest` cookie jar. | A compromised local process with debugger access to this one could still read the password from memory during the brief window it's held; same threat model as any local desktop client. |
