...
| Direct Ollama web lookup | Info disclosure; prompt injection through web text | Runs only when `web_intent.rs` finds an explicit phrase at the start of the message ("search the web for", "look up", "google", "read <link>", …); every other message never reaches a web route. At most one lookup per message, sent to the local daemon's `POST /api/experimental/web_search` or `/api/experimental/web_fetch` on `127.0.0.1:11434` (allowlisted in `ollama.rs`, redirects refused). Avatar sends no key or auth header; the daemon signs the upstream request with its own `ollama signin` identity. A link must be public `http(s)` with no credentials. Loopback, private, link-local, CGNAT, documentation, `.local`/`.internal`/`.lan`, and single-label hosts are refused, and fragments are dropped. Responses are capped at 1 MiB, then clipped (5 results × 700 chars; pages at 6,000 chars). They reach the model as a delimited block marked untrusted, with `<`/`>` neutralized so the text can't close the block early. The model has no tools, so web text can influence only the reply text, which still renders as plain text and is never opened. `tests/source_contracts.rs::ollama_web_lookups_stay_on_the_loopback_daemon` locks the routes and bans cloud hosts, keys, and tools in `ollama.rs`. | The query or link leaves the machine through Ollama's cloud service under the user's Ollama account. Web text can still mislead the answer. The routes are experimental and may change; failures surface as `web lookup unavailable (http N)` rather than a silent offline answer. |
...
Default chat backend. Same fail-closed HTTP rules as harness chat: `127.0.0.1:11434` only, no redirects, no forwarding headers, no API key, model tag is a **constant** (`qwen3.8:27b-mlx`). Path allowlist: `/v1/chat/completions`, `/api/tags`, and the daemon's experimental `/api/experimental/web_search` and `/api/experimental/web_fetch`. The bundled Soul Markdown is sent as the first `system` message on Direct Ollama chat requests only. It is application data, not tool authority: the request has no `tools`, filesystem, process, or write capability. Web lookups happen only on an explicit user request, through the local daemon; see the "Direct Ollama web lookup" row above. Does not call `/api/generate`, Ollama's cloud APIs directly, or any harness write route.
